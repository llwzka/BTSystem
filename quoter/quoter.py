...
class Quoter:
    ...
    def get_test_date(self, test_info):
        ...
        tk_end_date = pd.to_datetime(
            self.tickers[test_info.contract_name].date[len(self.tickers[test_info.contract_name].date) - 1])
    ...
    def get_customized_tickers_min(self, customized_tickers, test_tickers, test_info):  # test_tickers的每个元素都是series类型
        start_time = datetime.datetime.strptime(test_tickers.date.values[0] + ' ' + test_tickers.time.values[0],
                                                "%Y/%m/%d %H:%M:%S")
        ...
        for i in range(len(test_tickers.time)):
            ...
            ticker.time, ticker.date, ticker.close, ticker.high, ticker.low, ticker.open, ticker.vol = \
            test_tickers.time.values[i], test_tickers.date.values[i], test_tickers.close.values[i], \
            test_tickers.high.values[i], test_tickers.low.values[i], test_tickers.open.values[i], \
            test_tickers.vol.values[i]
            ticker_timenow = datetime.datetime.strptime(test_tickers.date.values[i] + ' ' + test_tickers.time.values[i],
                                                        "%Y/%m/%d %H:%M:%S")
            if ((start_time < end_time) and (ticker_timenow > end_time)) or (
                    (start_time < end_time) and (ticker_timenow < start_time)) or (
                    (start_time > end_time) and (ticker_timenow < start_time) and (ticker_timenow > end_time)):
